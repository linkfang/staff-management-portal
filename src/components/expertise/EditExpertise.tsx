  ...
  return (
    <>
      ...
      <div css={{ position: 'relative' }}>
        <div css={{ display: 'flex', gap: 20, flexWrap: 'wrap', opacity: isFetching ? 0.3 : 1 }}>
          {data?.map((item) => {
        ...
        {isFetching && (
          <Spin
            css={{ position: 'absolute', inset: 0, display: 'flex', justifyContent: 'center', alignItems: 'center' }}
          />
        )}
      </div>

      <div css={{ marginTop: 30, display: 'flex', justifyContent: 'flex-end', gap: 20 }}>
        <Button disabled={isMutating} onClick={() => closeModal()}>
          Cancel
        </Button>

        <Button type="primary" loading={isMutating} onClick={() => callbackFunc(editingData)}>
          Save
        </Button>
      </div>
    </>
